<div>
    ...
    <x-jet-confirmation-modal wire:model="isModalOpen">
        <x-slot name="title">
            Delete <span class="font-bold">{{ $title }}</span>
        </x-slot>

        <x-slot name="content">
            Are you sure you want to delete <span class="font-bold">{{ $title }}</span>?
        </x-slot>
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="ml-2" wire:click="deleteContentFile" wire:loading.attr="disabled">
                Yes
            </x-jet-danger-button>
        </x-slot>
